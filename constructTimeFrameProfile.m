...
function[output_data] = constructTimeFrameProfile(input_data, p)
    ...
    datasize = p.nChannels*p.nFrames*p.nChirpLoops*p.nSamples;
%   comp = p.nChannels*p.nFrames*p.nChirps*p.nSamples
%   chan = p.nChannels
%   fram = p.nFrames
%   chir = p.nChirps
%   sam = p.nSamples
    %Determine whether the input data has the correct number of values
    if input_size ~= p.nChannels*p.nFrames*p.nChirpLoops*p.nSamples
        %Throw error
        error("Error: Data array does not contain the correct number of samples.")
    end
    %Define the empty output array
    output_data = complex(zeros(p.nChannels, p.nFrames, p.nChirpLoops, p.nSamples));
    
    %Reshape the unordered data into a 4-dimensional array
    input_data = reshape(input_data, p.nSamples, p.nChannels, p.nChirpLoops, []);
    
    %Rearrange the indices
    for w = 1:p.nChannels
        for x = 1:p.nFrames
            for y = 1:p.nChirpLoops
                for z = 1:p.nSamples
